 ...
 *   3. DSH 凭据存储（`ctx.credentials`）的 ref TYPESAFE_AI_API_KEY
 *
 ...
 *   3. `ctx.credentials.resolve('TYPESAFE_AI_API_KEY')`
 *
 * 第 3 层只问 DSH 的凭据存储：它按 YAML 语义解析并自带分层覆盖（继承环境 > 项目 .env >
 * 用户 .env > 凭据文件），来源见 `ResolvedCredential.source`。不要改回自己读
 * `$DSH_HOME/.credentials.yaml`。
 *
 * @param config - 插件配置，可能带 apiKey。
 * @param credentials - `ctx.get('credentials')`；未挂载凭据存储时为 undefined。
...
export async function apply(ctx, config = {}) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			// 惰性取服务：凭据提供方与本插件是两条独立的 patch 行，装配顺序不保证。
			const apiKey = await resolveKey(cfg, ctx.get('credentials'));
			if (!apiKey) {
				throw new Error('jev_decide: 未找到 Typesafe API key（按序检查：插件 config.apiKey → 环境变量 TYPESAFE_API_KEY → DSH 凭据存储 ref TYPESAFE_AI_API_KEY）');
			}
